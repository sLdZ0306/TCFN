...
from util.data_util import index_to_time, index_to_time_5r
...
def calculate_iou_accuracy_5r(ious, threshold, k):

    total_size = float(len(ious)) / k
    count = 0
    flag = 0
    idx = 0
    for iou in ious:
    ...
    # for i in range(int(total_size * k), k):
    #     for j in range(5):
...
def eval_test(model, data_loader, device, max_len, mode='test', epoch=None, global_step=None):
    ious = []
    ious_5r = []
    flag = 0
    with torch.no_grad():
        for idx, (records, vfeats, vfeat_lens, word_ids, char_ids) in tqdm(
            ...
            # print("eval_videofeature: ", vfeats)
            word_ids, char_ids = word_ids.to(device), char_ids.to(device)
            # generate mask
            query_mask = (torch.zeros_like(word_ids) != word_ids).float().to(device)
            video_mask = convert_length_to_mask(vfeat_lens).to(device)
            # compute predicted results
            p, start_logits, end_logits = model(word_ids, char_ids, vfeats, video_mask, query_mask)
            ...
            for record, start_index, end_index in zip(records, start_indices, end_indices):
                start_time, end_time = index_to_time(start_index, end_index, max_len, record["duration"])
                iou = calculate_iou(i0=[start_time, end_time], i1=[record["s_time"], record["e_time"]])
                ious.append(iou)
                # for visualization result
                # if iou >= 0.70 and flag == 0:
                #     gts, gte = record["s_time"], record["e_time"]
                #     st, et = start_time, end_time
                #     si, ei = start_index, end_index
                #     egiou = iou
                #     video_idx = record
                #     flag = 1
                #     print("gts, gte: ", gts, gte)
                #     print("st, et: ", st, et)
                #     print("si, ei: ", si, ei)
                #     print("egiou: ", egiou)
                #     print("video_idx: ", video_idx)
                # if record["sample_id"] == 14237 and flag == 0:
                #     gts, gte = record["s_time"], record["e_time"]
                #     st, et = start_time, end_time
                #     si, ei = start_index, end_index
                #     egiou = iou
            ...
            start_indices_5r, end_indices_5r = model.extract_index_5r(start_logits, end_logits)
            start_indices_5r = start_indices_5r.cpu().numpy()
            end_indices_5r = end_indices_5r.cpu().numpy()
            for record, start_index, end_index in zip(records, start_indices_5r, end_indices_5r):
                start_time, end_time = index_to_time_5r(start_index, end_index, max_len, record["duration"])
                for st, et in zip(start_time, end_time):
                    iou_5r = calculate_iou(i0=[st, et], i1=[record["s_time"], record["e_time"]])
                    ious_5r.append(iou_5r)
    # print(len(ious))
                # ious.append(iou)
    ...
    r5i3 = calculate_iou_accuracy_5r(ious_5r, threshold=0.3, k=5)
    r5i5 = calculate_iou_accuracy_5r(ious_5r, threshold=0.5, k=5)
    r5i7 = calculate_iou_accuracy_5r(ious_5r, threshold=0.7, k=5)
    # print(ious_5r)
    # for visualization result
    ...
    return r1i3, r1i5, r1i7, r5i3, r5i5, r5i7, mi, score_str
