...
def visual_feature_sampling(visual_feature, max_num_clips):
    ori_video_len = visual_feature.shape[0]
    index = np.linspace(start=0, stop=ori_video_len - 1, num=max_num_clips).astype(np.int32)
    new_video = []
    for i in range(len(index) - 1):
        start = index[i]
        end = index[i + 1]
        if start == end or start + 1 == end:
            new_video.append(visual_feature[start])
        else:
            new_video.append(np.mean(visual_feature[start: end], 0))
    new_video.append(visual_feature[-1])
    visual_feature = np.stack(new_video, 0)
...
def index_to_time_5r(start_index, end_index, num_units, duration):
    # print('start: ', start_index)
    # print('end: ', end_index)
    # print('num: ', num_units)
    # print('dur: ', duration)
