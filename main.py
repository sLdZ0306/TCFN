...
from model.TCFN import TCFN, build_optimizer_and_scheduler
...
parser.add_argument('--fv', type=str, default='org', help='[new | org] for visual features')
parser.add_argument('--max_pos_len', type=int, default=200, help='maximal position sequence length allowed')
# model parameters
...
parser.add_argument("--video_feature_dim", type=int, default=500, help="video feature input dimension")
...
parser.add_argument("--highlight_lambda", type=float, default=4.0, help="lambda for highlight region")
...
parser.add_argument('--model_name', type=str, default='tcfn', help='model name')
...
if configs.mode.lower() == 'train':
    ...
    # build model
    model = TCFN(configs=configs, word_vectors=dataset['word_vector']).to(device)
    ...
    for epoch in range(configs.epochs):
        ...
        for data in tqdm(train_loader, total=num_train_batches, desc='Epoch %3d / %3d' % (epoch + 1, configs.epochs)):
            ...
            # print("main_videofeature: ", vfeat_lens)
            ...
            # print('start: ', s_labels)
            # print('end: ', e_labels)
            # compute loss
            # h_score = h_score + start_logits + end_logits
            highlight_loss = model.compute_highlight_loss(h_score, h_labels, video_mask)
            total_loss = model.compute_loss(start_logits, end_logits, s_labels, e_labels) + configs.highlight_lambda * highlight_loss
            ...
            if global_step % eval_period == 0 or global_step % num_train_batches == 0:
                model.eval()
                r1i3, r1i5, r1i7, r5i3, r5i5, r5i7, mi, score_str = eval_test(model=model, data_loader=test_loader, device=device, max_len=configs.max_pos_len,
                                                            mode='test', epoch=epoch + 1, global_step=global_step)
                print('\nEpoch: %2d | mLoss: %.2f | r1i3: %.2f | r1i5: %.2f | r1i7: %.2f | mIoU: %.2f' % (
                    epoch + 1, loss_sum / loss_count, r1i3, r1i5, r1i7, mi), flush=True)
                loss_sum = 0
                loss_count = 0
    ...
    model.load_state_dict(torch.load(filename), False)
    model.eval()
    r1i3, r1i5, r1i7, r5i3, r5i5, r5i7, mi, _ = eval_test(max_len=configs.max_pos_len, model=model, data_loader=test_loader, device=device, mode='test')
    ...
    print("\n" + "\x1b[1;31m" + "Rank@5, IoU=0.3:\t{:.2f}".format(r5i3) + "\x1b[0m", flush=True)
    print("\x1b[1;31m" + "Rank@5, IoU=0.5:\t{:.2f}".format(r5i5) + "\x1b[0m", flush=True)
    print("\x1b[1;31m" + "Rank@5, IoU=0.7:\t{:.2f}".format(r5i7) + "\x1b[0m", flush=True)
