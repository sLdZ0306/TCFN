...
class DynamicRNN(nn.Module):
    def __init__(self, dim):
        ...
        self.lstm = nn.LSTM(input_size=dim, hidden_size=dim, num_layers=1, bias=True, batch_first=True) #, bidirectional=default
...
class ConditionedPredictor(nn.Module):
    def __init__(self, dim, num_heads, max_pos_len, drop_rate=0.0, predictor='rnn'):
        ...
        self.end_block = nn.Sequential(
            Conv1D(in_dim=3 * dim, out_dim=dim, kernel_size=1, stride=1, padding=0, bias=True),
            nn.ReLU(),
            Conv1D(in_dim=dim, out_dim=1, kernel_size=1, stride=1, padding=0, bias=True)
        )
    ...
    def forward(self, x, mask):
        ...
        c = torch.cat([start_features, end_features, x], dim=2)
        start_features = self.start_block(torch.cat([start_features, x], dim=2))  # (batch_size, seq_len, 1)
        # print("sf: ", start_features.size())
        # print("ef: ", end_features.size())
        # print("x: ", x.size())
        end_features = self.end_block(c)
    ...
    @staticmethod
    def extract_index_5r(start_logits, end_logits):
        top = 5
        start_prob = nn.Softmax(dim=1)(start_logits)    # bs len
        ...
        bs = outer.shape[0]
        sl = outer.size(1)
        outer_1d = outer.view(bs, sl*sl)
        # print(outer_1d.shape)
        _, index = torch.topk(outer_1d, k=top, dim=1)  # (batch_size, )
        start_index = torch.ceil(index / sl) - 1
        end_index = index - torch.floor(index / sl) * sl - 1
        # print(end_index)
        # _, end_index = torch.topk(outer, k=5, dim=1)  # (batch_size, )
        # print()
        return start_index, end_index
